...
Software for imaging the Faraday spectrum, i.e. the 3D distribution of polarized
intensity as a function of Faraday depth and position on the sky. Imaging is 
conducted using the Faraday synthesis technique (see Bell and Ensslin (2012) for 
details) and therefore inherently in 3D. Deconvolution is carried out using a 
3D Clark CLEAN algorithm. 

Data is read from MeasurementSet files of the type used by CASA. Images are 
written to FITS files.

CHANGELOG:
    14.09.2012      0.1.0.0     Complete rewrite of the test code used for 
                                the paper, designed to be more
                                memory friendly. CLEAN has not been implemented in
                                this version. Only a dirty image can be produced.
"""
...
class FSCoords(object):
    ...
    def __init__(self, pm):
        """
        Takes a parset manager class instance and computes all coordinate values
        required.
...
class FSClean(object):
    ...
    def __init__(self, pm=None):
        """
        Initialize the FSClean imager. Sets common values and inits the messenger
        class.
        
        Args:
            parset: FSCleanPM class instance, with the parset dict already loaded
    ...
    def run(self, msfn, outfn_base):
        ...
        weights = FSCData(self.sfnbase+'_weights.hdf5', np.dtype('float64'), \
            m=self.m)
        self.register_scratch_files([weights.fn, weights.coords.fn])
        
        vis = FSCPolData(self.sfnbase+'_vis.hdf5', coords=weights.coords, \
            m=self.m)
        ...
        l2min = l2min - self.coords.grid_params.W*2.*im.fourier_grid.deltas[0]
#        if l2min < 0.:
#            l2min = 0.
        ...
        # Hand off data to the CLEAN function
    ...
    def set_normalizations(self, weights, db):
        ...
        Args: 
            weights: An FSCData object containing the weights for each visibility.
        ...
        if self.do_clean:
            ...
            temp = FSCData(self.sfnbase+'_tempdata.hdf5', coords=weights.coords, \
                dtype=np.dtype('float64'), m=self.m, template=weights)
    ...
    def clark_clean(self, vis, weights, im, db):
        ...
        Returns:
            ...
            model locations (phi, dec, ra) defined in pixels, and the model flux.
        
        """
        
        self.m.header2("Started the CLEAN routine...")
        ...
        if not self.do_clean:
            vis.transform(im) # im will contain the residual image going forward
            im.multiplywith(self.K) 
            return None, None
            
        # object for holding the model point source image
        pointim = FSCImage(self.sfnbase+'_pointim.hdf5', np.dtype('complex128'), \
            self.coords.grid_def, self.coords.grid_params, m=self.m)
        ...
        patch = tdb.im[nphi/2-pnphi/2:nphi/2+pnphi/2, nm/2-pnm/2:nm/2\
            +pnm/2, nl/2-pnl/2:nl/2+pnl/2]
    
        # get only the rest of the beam outside of the patch
#        tdb.im[nphi/2-pnphi/2:nphi/2+pnphi/2, nm/2-pnm/2:nm/2\
#            +pnm/2,nl/2-pnl/2:nl/2+pnl/2] =tdb.im[nphi/2-pnphi/2:nphi/2+pnphi/2,\
#            nm/2-pnm/2:nm/2+pnm/2, nl/2-pnl/2:nl/2+pnl/2] - patch
        tdb.im[nphi/2-pnphi/2:nphi/2+pnphi/2, nm/2-pnm/2:nm/2\
            +pnm/2,nl/2-pnl/2:nl/2+pnl/2] = np.zeros((pnphi, pnm, pnl), \
        ...
        self.m.message("Largest sidelobe level outside beam patch: "+str(extsl),3)
        ...
        while True:
            ...
            self.m.message("Initial minor cycle stop level: "+str(slim*F),2)
            ...
            # Minor cycle
                ...
                self.m.message(".    .    abs. value: " + str(abs(pval*gain)),2)
                ...
                # crop the patch if necessary (because it runs off the map edge)
                phimax = pphi + pnphi/2
                phimin = pphi - pnphi/2
                pc_phi_low = 0
                pc_phi_high = pnphi            
                ...
                tpatch=patch[pc_phi_low:pc_phi_high, \
                    pc_m_low:pc_m_high, pc_l_low:pc_l_high].copy()
                    
                im.im[phimin:phimax, mmin:mmax, lmin:lmax] = \
                    im.im[phimin:phimax, mmin:mmax, lmin:lmax] - gain*pval*tpatch
                ...
                if abs(pval)<cutoff or N>niter:
                    ...
                    self.m.success("Stopping! Cutoff or niter has been reached.")
                    stop=True
                    break
            ...
            self.m.message("Inverting model to vis. space and subtracting...",1)
        ...
        resim = FSCImage(self.sfnbase+'_resim.hdf5', np.dtype('complex128'), \
            self.coords.grid_def, self.coords.grid_params, m=self.m)
    ...
    def make_beam_image(self, beamim):
        ...
        # the size of the image over which to compute the gaussian, zero outside
        denom=self.pm.parset['beam_patch_frac']
        
        [nphi, nm, nl] = beamim.im.shape

        patch = np.zeros((nphi/denom, nm/denom, nl/denom), dtype=beamim.im.dtype)
        ...
        for i in range(patch.shape[0]):
            for j in range(patch.shape[1]):
                for k in range(patch.shape[2]):
                    patch[i,j,k] = np.exp(-0.5*(invsigmaphi2*(i-phic)**2 \
                        + invsigmam2*(j-mc)**2 + invsigmal2*(k-lc)**2 ))
    ...
    def make_cclist_image(self, cclist, im):
        ...
        for i in range(len(cclist)):
            ...
            im.im[phi,m,l] = im.im[phi,m,l] + val # there must be a better way...
...
if __name__ == '__main__':
    ...
    desc = "Software for reconstructing the Faraday spectrum, i.e. the 3D "+\
        ...
        "Images are written to HDF5 image files. See the README for info about "+\
    ...
    parser.add_option("-p", "--parset_desc", action="store_true", \
        help="show parameter set file description and exit", default=False)    
