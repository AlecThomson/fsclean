...
class FSCImage(PI):
    ...
    def convolve_with(self, other):
        ...
        for i in range(self.osim.shape[1]):
            ...
            for j in range(self.osim.shape[2]):
                ...
                oim_slice[:,j] = np.fft.ifft(np.fft.ifftshift(oim_slice[:,j]))
    ...
    def transform(self, data):
        ...
        transformation and degridding. The transformed Data will not be properly
        ...
        if not isinstance(data, FD.FSCData) \
            and not isinstance(data, FD.FSCPolData):
                raise Exception("Unsupported RAData object used as target for"+\
                    " image transform!")
        ...
        for i in data.iterkeys():
            ...
            for j in range(nchan):
                ...
                l2ndx = int((l2 - self.fourier_grid.mincoords[0] + \
                    0.5*self.fourier_grid.deltas[0])/self.fourier_grid.deltas[0])
...
#                if slab_min < 0:
#                    slab_min = 0
#                if slab_max > self.fourier_grid.shape[0]:
#                    slab_max = self.fourier_grid.shape[0]
                
                if slab_min >= self.fourier_grid.shape[0] or slab_min < 0 \
                    or slab_max < 0 or slab_max >= self.fourier_grid.shape[0]:
                        raise Exception("Something went wrong when calculating"+\
                            " the lambda^2 index.")
...
#                if min(uvec) <= umin+0.5*W*du or min(vvec) <= vmin+W*dv*0.5 \
#                    or max(vvec) >= vmin+dv*Nv-W*dv*0.5 \
#                    or max(uvec) >= umin+du*Nu-W*du*0.5 \
#                    or l2 >= slab_l2min+dl2*len(fg_slab)-W*dl2*0.5\
#                    or l2 <= slab_l2min+W*dl2*0.5:
#                        print "Error will probably occur here: " 
#                        print "inputs: ", i, j, max(uvec), min(uvec), max(vvec), \
#                                min(vvec), l2, du, Nu, umin, dv, Nv, vmin, dl2,\
#                                len(fg_slab), slab_l2min
